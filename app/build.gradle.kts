plugins {
    alias(libs.plugins.android.application)
    alias(libs.plugins.kotlin.android)
    alias(libs.plugins.kotlin.compose)
    alias(libs.plugins.hilt.android)
    alias(libs.plugins.ksp)
}

android {
    namespace = "com.example.photodiary"
    compileSdk = 35

    defaultConfig {
        applicationId = "com.example.photodiary"
        minSdk = 25
        targetSdk = 35
        versionCode = 1
        versionName = "1.0"

        testInstrumentationRunner = "androidx.test.runner.AndroidJUnitRunner"
    }

    buildTypes {
        release {
            isMinifyEnabled = false
            proguardFiles(
                getDefaultProguardFile("proguard-android-optimize.txt"),
                "proguard-rules.pro"
            )
        }
    }

    compileOptions {
        sourceCompatibility = JavaVersion.VERSION_17
        targetCompatibility = JavaVersion.VERSION_17
    }

    kotlinOptions {
        jvmTarget = "17"
    }

    buildFeatures {
        compose = true
    }
}

// Guard against IDE-created *RuntimeClasspathCopy configurations behaving as both
// resolution roots and outgoing variants.
fun org.gradle.api.artifacts.Configuration.enforceResolutionOnlyRole() {
    isCanBeResolved = true
    isCanBeConsumed = false
    isVisible = false
}

configurations.configureEach {
    if (name.endsWith("RuntimeClasspathCopy")) {
        enforceResolutionOnlyRole()
    }
}

configurations.whenObjectAdded {
    if (name.endsWith("RuntimeClasspathCopy")) {
        enforceResolutionOnlyRole()
    }
}

afterEvaluate {
    configurations.matching { it.name.endsWith("RuntimeClasspathCopy") }.configureEach {
        enforceResolutionOnlyRole()
    }
}


dependencies {
    implementation(libs.androidx.core.ktx)
    implementation(libs.androidx.lifecycle.runtime.ktx)
    implementation(libs.androidx.activity.compose)

    implementation(platform(libs.androidx.compose.bom))
    implementation(libs.androidx.ui)
    implementation(libs.androidx.ui.graphics)
    implementation(libs.androidx.ui.tooling.preview)
    implementation(libs.androidx.material3)

    implementation(libs.androidx.navigation.compose)

    implementation(libs.hilt.android)
    ksp(libs.hilt.compiler)
    implementation(libs.androidx.hilt.navigation.compose)

    implementation(libs.androidx.room.runtime)
    implementation(libs.androidx.room.ktx)
    ksp(libs.androidx.room.compiler)

    testImplementation(libs.junit)
    androidTestImplementation(libs.androidx.junit)
    androidTestImplementation(libs.androidx.espresso.core)
    androidTestImplementation(platform(libs.androidx.compose.bom))
    androidTestImplementation(libs.androidx.ui)

    debugImplementation(libs.androidx.ui.tooling)
}
